...
def chop_background(background_config: Dict[str, Tuple], video_length: int, text_object: dict):
    ...
    Args:
        ...
        text_object (dict): Object containing text_id for file organization
    """
    # Sanitize ID for safe path handling
    text_id = text_object.get("thread_id", "default")
    print(f"Text ID: {text_id}")
    safe_id = re.sub(r'[^\w-]', '_', text_id)
    print(f"safe ID: {safe_id}")
    temp_dir = Path(f"assets/temp/{safe_id}")
    print(f"temp dir(139): {temp_dir}")
    ...
    if settings.config["settings"]["background"][f"background_audio_volume"] == 0:
        ...
        print(f"temp dir(152): {temp_dir}")
        background_audio.write_audiofile(str(temp_dir / "background.mp3"))
    ...
    # Try to extract video subclip using VideoFileClip first
    try:
        with VideoFileClip(f"assets/backgrounds/video/{video_choice}") as video:
            new = video.subclip(start_time_video, end_time_video)
            new.write_videofile(str(temp_dir / "background.mp4"))
            print_substep("Background video chopped successfully!", style="bold green")
    except Exception as e:
        print_substep(f"Error during video processing: {str(e)}")
        raise
